...
# Nikki's WD
setwd("C:/Users/Nikki/Desktop/ra_research")
...
### Recode [Prefer not to answer] to NA

# value corresponding to [Prefer not to answer] for each applicable question
vals <- unlist(lapply(d, function(x){
  as.vector(attributes(x)$labels[names(attributes(x)$labels) == 
                                   "[Prefer not to answer]"])
}))

# change values to NA in dataset
for(var in names(vals)){
  d[d[, var] == vals[var] & !is.na(d[, var]), var] <- NA
}


#new task: if someone failed any attention check, delete all of their responses.
